...
public:
    ...
    bool connectToDevice(char *address, unsigned short myport=0, bool excl=false);
    bool connectionOK();
...
protected:
    ...
    int32 reading;
    static void writeStreamCallback(SOCKET *stream, int eventType, void *clientCallBackInfo);
    bool continueExec(bool ok);
    bool get_port2(bool ok);
    bool create_link2(bool ok);
    void connectToDevice2(bool ok);
    static char errstring[36];

    bool CreateSocketToHost(char *address, int port, SOCKET *sock);
};
